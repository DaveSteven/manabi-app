...
- 听力精听入口：默认模糊隐藏原文、醒目的原文显示按钮、单句播放后暂停、底部手动切句；无时间标记时提示返回整段。
...
`ManabiTests` 的 6 项测试覆盖服务地址校验、JSON 解码、幂等请求字段和重复答案不重复计数，整卷各专项进度汇总，以及实际音频的句尾停止和手动重播行为。`ManabiUITests` 的 4 项测试使用运行中的真实 API 验证强制登录、退出与会话恢复、完整答题、重启续练、阅读材料、听力播放、按试卷保存进度及拖动后播放条持续更新；测试 token 与普通使用的 token 分开保存。
...
题目图片、整段听力和精听共用 `ResourceCache`。先确认服务端资源版本，再使用本地文件或下载，
以字节大小和 SHA-256 验证完整性；缓存键隔离服务环境、资源 ID 与内容版本。
图片放大复用同一图片，播放器不在播放中途更换文件来源。零字节统一显示为 `0KB`。

当前题目允许移动网络加载；Wi-Fi、前台且非低数据模式时预加载后面最多三题。
媒体下载最多并发两个，预加载最多占一个，前台请求优先；切题、退出、网络变化会取消旧预加载，
仍被其他使用者需要的共享请求继续。元数据接口不可用时保留兼容在线加载及已有缓存复用。

「我的 → 缓存管理」只显示总大小、音频/图片占用与清理按钮。
不提供自动缓存容量上限、主动下载或批量下载网络设置。清理保留当前练习、正在下载和刚交付显示/播放的资源，
不影响学习记录。资源保存在系统 Caches 目录，可由系统回收；索引保存在 Application Support 并排除备份。
历史主动下载资源在首次读取索引时迁移为普通缓存，保留复用能力并支持统一清理。

这不是完整离线作答：获取题目、字幕和提交答案仍需要 API。
线上 `https://biblenotes.cc` 已部署资源清单接口并补齐资源哈希。
